...
define(
    ...
    "INSERT INTO " . TBL_COMPLAINTS_CONCERNS . " (student_id, type, severity, description, preferred_counseling_date, evidence, mime_type, audio_recording, audio_mime_type, audio_duration, status, date_created, time_created)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
);

define(
    'SQL_UPDATE_COMPLAINTS_CONCERNS',
    "UPDATE " . TBL_COMPLAINTS_CONCERNS . " SET student_id = ?, type = ?, severity = ?, description = ?, preferred_counseling_date = ?, evidence = ?, mime_type = ?, audio_recording = ?, audio_mime_type = ?, audio_duration = ?, status = ?, date_created = ?, time_created = ? WHERE id = ?"
);
